<template>
  <div class="promotion" id="promotion">
    <!-- 头部 -->
    <div class="header"><i class="iconfont icon-zuo" @click="goBack"></i>商城促销</div>
    <!-- 分类 -->
    <div class="kindList swiper-container">
      <div class="swiper-wrapper">
        <ul>
          <li @click="choiceFL(0)" :class="{active:cid==0}">
            <div class="swiper-slide">全部</div>
          </li>
          <li v-for="item in flashSale" @click="choiceFL(item.id)" :class="{active:cid==item.id}">
    ...
    <ul class="goods" v-for="good in goodsList">
      <li>
        ...
        <div class="goodsDespriction">
          ...
          <p class="sales">已售{{good.sell_num}}
            <!--<span><i :style="{width:good.sell_num/good.max_num*100+'px'}"></i></span>-->
            <button class="buy" @click="toBuy(good.id)">立即抢购</button>
          </p>
        </div>
      </li>
    </ul>
  ...
  export default {
    ...
    methods: {
      ...
      getGoodsList(add) {
        ...
        this.axios.post(this.getApi('web/prom_goods'),
          this.dafa({
      ...
      pullUp() {
        ...
        let top = document.body.scrollTop || document.documentElement.scrollTop,
          ...
          appHeight = document.getElementById('promotion').scrollHeight;
