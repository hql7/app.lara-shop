            ...
            <router-link :to="'/products?id='+item.id+'&pid='+item.product_id">
              <div class="m-Sbox">
                ...
                <div class="u-Scontent">
                  <h3>{{item.name}}</h3>
                  <p>￥{{item.real_price}}<span class="u-right" @click.prevent="delLog(item.id,item.product_id)"><i
                    class="iconfont icon-shanchu2"></i></span></p>
                </div>
              </div>
            </router-link>
          </li>
  ...
  export default {
    data() {
      return {
        ...
        type: {
          collect: 0, // 收藏类型  0=商品收藏  店铺收藏先不做
          good: 0  // 商品类型  0=全部商品  1=活动商品
        },
        isLoading: true
      }
    },
    created: function () {
      this.get_attention(); //加载完成获取我 的收藏
    },
    mounted: function () {
    ...
    methods: {
      ...
      get_attention: function (add) {
        ...
        this.axios.post(this.getApi('web/my_attention'),
          this.dafa({
            user_id: this.getobjS('user').id,
            token: this.getobjS('user').token,
            index: this.page.index,
            collect_type: this.type.collect,
            good_type: this.type.good
          })
        ).then(res => {
